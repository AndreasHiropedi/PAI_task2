...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.mean_params = {}
        self.squared_mean_params = {}
        self.cov_matrix = {}
        self.num_samples = 0

        # Initialize covariance factors for SWAG low-rank approximation
        self.cov_factor_1 = {name: torch.zeros_like(param) for name, param in self.network.named_parameters()}
        self.cov_factor_2 = {name: torch.zeros_like(param) for name, param in self.network.named_parameters()}
        ...
        if self.inference_mode == InferenceType.SWAG_FULL:
            ...
            for name, param in copied_params.items():
                if name not in self.mean_params:
                    self.mean_params[name] = torch.zeros_like(param)
                    self.squared_mean_params[name] = torch.zeros_like(param)

                # Update mean and squared mean
                self.mean_params[name] = (self.mean_params[name] * self.num_samples + param) / (self.num_samples + 1)
                self.squared_mean_params[name] = (
                    self.squared_mean_params[name] * self.num_samples + param ** 2
                ) / (self.num_samples + 1)

                deviation = (param - self.mean_params[name]).detach()
    
                # Update cov_factor_1 and cov_factor_2 to capture low-rank directions
                if self.num_samples % 2 == 0:
                    self.cov_factor_1[name] = deviation
                else:
                    self.cov_factor_2[name] = deviation
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.num_bma_samples, desc="Performing Bayesian model averaging"):
            ...
            for (batch_images,) in loader:
                with torch.no_grad():
                    predictions = torch.softmax(self.network(batch_images), dim=1)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            if self.inference_mode == InferenceType.SWAG_FULL:
                ...
                v1 = self.cov_factor_1[name]  # First low-rank direction
                v2 = self.cov_factor_2[name]  # Second low-rank direction

                # Sample Gaussian coefficients for low-rank vectors
                z1 = torch.randn(1)
                z2 = torch.randn(1)

                # Add Full SWAG contribution to the sampled weights
                sampled_weight += v1 * z1 + v2 * z2
